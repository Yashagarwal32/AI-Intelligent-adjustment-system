...
class Application:
  ...
  #=========testing===============
    ...
    def student_entry(self):
        ...
        #conn.commit()
        '''
        c.execute("create table llslot1(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        conn.commit()
        c.execute("create table llslot2(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        conn.commit()
        c.execute("create table llslot3(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        conn.commit()
        c.execute("create table llslot4(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        conn.commit()
        c.execute("create table llslot5(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        conn.commit()
        c.execute("create table llslot6(uid text default 'NULL',section text default 'NULL',subject text default 'NULL')")
        '''
        ...
        if self.input1=="" or self.input2=="":
            ...
            if c.fetchone() is  not None :
                ...
                if c.fetchone() is  not None :
                    ...
                    #c.execute("create table lslot1(uid text,section text,subject text)")
                    #c.execute("create table lslot2(uid text,section text,subject text)")
                    #c.execute("create table lslot3(uid text,section text,subject text)")
                    #c.execute("create table lslot4(uid text,section text,subject text)")
                    #c.execute("create table lslot5(uid text,section text,subject text)")
                    #c.execute("create table lslot6(uid text,section text,subject text)")
                    ...
                    for i in range(len(y9)):
                        ...
                        for b in t2:
                            ...
                            if b[0] in y9[i]:
                                ...
                                sr31=c.execute('SELECT subject1,subject2,subject3,subject4 from teachertime1 where uid="%s"' %(b[i]))
                                sr32=c.fetchone()
                                for p in w11:
                                    for q in sr32:
                                        if p==q:
                                            ...
                                            for f in sr34:
                                                if g in f:
                                                    ...
                                                    if oo==1 or pp==1:
                                                        ...
                                                        ll1=c.fetchall()
                                                        lt=c.execute('SELECT uid from llslot1')
                                                        lt1=c.fetchall()
                                                        
                                                        if(f[0] not in ll1 and f not in lt1):
                                                            sql1="INSERT INTO 'llslot1' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql1, (f[0],t4[0],g))
                                                            conn.commit()
                                                    ...
                                                    if oo==2 or pp==2:
                                                        ...
                                                        ll1=c.fetchall()
                                                        lt=c.execute('SELECT uid from llslot2')
                                                        lt1=c.fetchall()
                                                        
                                                        if(f[0] not in ll1 and f not in lt1):
                                                            sql2="INSERT INTO 'llslot2' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql2, (f[0],t4[0],g))
                                                            conn.commit()
                                                    ...
                                                    if oo==3 or pp==3:
                                                        ...
                                                        ll1=c.fetchall()
                                                        print(ll1)
                                                        lt=c.execute('SELECT uid from llslot3')
                                                        lt1=c.fetchall()
                                                        print(lt1)
                                                        print(f[0])
                                                        print(f[0] not in ll1)
                                                        print(f[0] not in lt1)
                                                        
                                                        
                                                        if(f[0] not in ll1 and f[0] not in lt1):
                                                            sql3="INSERT INTO 'llslot3' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql3, (f[0],t4[0],g))
                                                            conn.commit()
                                                            self.box1.insert(END,"Teacher is on adjustment with id "+f[0]+"\nhaving subject  : "+g+"at 11am-12pm"+"\n")   
                                                    if oo==4 or pp==4:
                                                        ll=c.execute('SELECT slot4 from sectiontime')
                                                        ll1=c.fetchall()
                                                        lt=c.execute('SELECT uid from llslot4')
                                                        lt1=c.fetchall()
                                                        
                                                        if(f[0] not in ll1 and f not in lt1):
                                                            sql4="INSERT INTO 'llslot4' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql4, (f[0],t4[0],g))
                                                            conn.commit()
                                                            self.box1.insert(END,"Teacher is on adjustment with id "+f[0]+"\nhaving subject  : "+g+"at 12pm-1pm"+"\n")   
                                                    if oo==5 or pp==5:
                                                        ll=c.execute('SELECT slot5 from sectiontime')
                                                        ll1=c.fetchall()
                                                        lt=c.execute('SELECT uid from llslot5')
                                                        lt1=c.fetchall()
                                                        
                                                        if(f[0] not in ll1 and f not in lt1):
                                                            sql5="INSERT INTO 'llslot5' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql5, (f[0],t4[0],g))
                                                            conn.commit()
                                                    ...
                                                    if oo==6 or pp==6:
                                                        ...
                                                        ll1=c.fetchall()
                                                        lt=c.execute('SELECT uid from llslot6')
                                                        lt1=c.fetchall()
                                                        
                                                        if(f[0] not in ll1 and f not in lt1):
                                                            sql6="INSERT INTO 'llslot6' (uid,section,subject) VALUES(?,?,?)"
                                                            c.execute(sql6, (f[0],t4[0],g))
                                                            conn.commit()
        ...
        if self.inteacher1=="" or self.inteacher2=="":
            ...
            if c.fetchone() is  not None :
                ...
                if c.fetchone() is  not None :
                    ...
                    self.applyleave=Button(self.teachere1,text="Apply Leave",width=20,height=2,bg='steelblue',command=self.leave_db)
    ...
    def leave_db(self):
        ...
        if self.vari1=="":
            ...
            p2=c.fetchone()
            self.box2.insert(END,"Your leave has successfully approved")
            '''
            y8=c.execute('SELECT slot1,slot2,slot3,slot4,slot5,slot6 from sectiontime')
            y9=c.fetchall()
            for a in y9:
                    if(self.inteacher1 in a):
                        self.box1.insert(END,"Teacher on leave with id : ",self.inteacher1) 
